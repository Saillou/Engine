...
void MaterialEditor::onEnter()
{
    ...
    m_entity->setPoses({ glm::scale(glm::mat4(1.0f), glm::vec3(0.15f)) });
...
{
    m_entity->material.diffuse_color = glm::vec4(m_menu.state.color.x, m_menu.state.color.y, m_menu.state.color.z, m_menu.state.color.w);
