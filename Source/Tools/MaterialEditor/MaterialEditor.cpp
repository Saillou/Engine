...
{
    UShader& shader = m_entity->get(Cookable::CookType::Basic);
    shader->use();
    ...
    if (m_menu.state.lightEnabled)
        m_entity->drawOne(Cookable::CookType::Basic, m_window->scene()->camera(),glm::scale(glm::mat4(1.0f), glm::vec3(0.15f)), m_lights);
    else
        m_entity->drawOne(Cookable::CookType::Basic, m_window->scene()->camera(), glm::scale(glm::mat4(1.0f), glm::vec3(0.15f)));
