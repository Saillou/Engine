    ...
    m_scene(Service<Window>::get().scene())
{
    ...
    for (float x = -Ctx.Ground_Size /2.0f; x < Ctx.Ground_Size /2.0f; x++) {
        for (float y = -Ctx.Ground_Size /2.0f; y < Ctx.Ground_Size /2.0f; y++) {
            m_models["earth"]->transforms.push_back(pose(glm::vec2(x, y)));
        }
    }
    ...
    for (int i = 0; i < Ctx.Grass_Number; i++) {
        ...
        m_models["grass"]->transforms.push_back(pose_scale(glm::vec2(x, y), s));
    }

    // Only one train
    m_models["train"]->transforms = {
...
void SampleTrain::_compute_physics() {
    ...
    // Update model
    m_models["train"]->transforms.front() = pose_rot(m_player_data.position, m_player_data.angle);
