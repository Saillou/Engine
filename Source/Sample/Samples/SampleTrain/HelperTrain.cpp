...
Model::Ref HelperTrain::tile_with_texture(const std::string& texturePath) {
    ...
    model->localMaterial = glm::vec4(0, 0, 0, 0.9f);
...
Model::Ref HelperTrain::tile_with_rgba(const glm::vec4& colorRGBA) {
    Model::Ref model = Model::Create(Model::Quad);
    ...
    model->localMaterial = colorRGBA / 255.0f;
