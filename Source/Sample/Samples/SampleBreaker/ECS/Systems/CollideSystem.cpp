...
void CollideSystem::update(float dt)
{
    for (auto& entity : m_entities)
    {
        ...
        hitbox->localMaterial = { 0.7f, 0.2f,0.2f,1.f };
        hitbox->transforms = {
            {
                transform.getMat4() *
                collider.collisionBox->localTransform *
        ...
        for (auto& anotherEntity : m_entities)
        {
            ...
            anotherCollider.collisionBox->transforms = { {anotherTransform.getMat4()} };

            Model::Ref hitboxanother = Model::Create(Model::Cube);
            hitboxanother->localMaterial = { 0.2f, 0.2f,0.7f,1.f };
            hitboxanother->transforms = {
                {
                    anotherTransform.getMat4() *
                    anotherCollider.collisionBox->localTransform *
            ...
            if (collisionPoint.has_value()) 
            {
                ...
                point->localMaterial = { 0.7f, 0.2f,0.2f,1.f };
                point->localTransform = glm::scale(glm::mat4(1.f), glm::vec3(0.1f));
                point->transforms.push_back(glm::translate(glm::mat4(1.f), collisionPoint.value()));
