    ...
    m_scene(Service<Window>::get().scene())
{
    ...
    m_entities["particle"].localMaterial().diffuse_color = glm::vec4(1.0f, 0.2f, 0.9f, 0.9f);
    ...
    m_entities["particle_custom"].localMaterial().diffuse_color = glm::vec4(1.0f, 0.7, 0.9f, 0.4f);
...
// Particules methods
void SamplePong::_generate_particle(std::deque<_particle>& container, const _conditions& cnd)
{
    container.push_back({
...
void SamplePong::_update_particle(std::deque<_particle>& container, std::vector<Pose>& entity_poses)
{
...
// Shader methods
void SamplePong::_add_custom_shader(const std::string& name)
{
    // Add to memory
    _create_shader(name);
...
void SamplePong::_create_shader(const std::string& name)
{
    m_shaders[name]
